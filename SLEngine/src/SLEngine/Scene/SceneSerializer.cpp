...
namespace YAML {
	...
	template<>
	struct convert<SLEngine::UUID>
	{
		static Node encode(const SLEngine::UUID& uuid)
		{
			Node node;
		...
		static bool decode(const Node& node, SLEngine::UUID& uuid)
		{
...
#define READ_SCRIPT_FIELD(FieldType, Type)             \
	...
	{
		SL_CORE_ASSERT(entity.HasComponent<IDComponent>());
	...
	bool SceneSerializer::Deserialize(const std::string& filepath)
	{
		...
		{
			SL_CORE_ERROR("Failed to load .SLEngine file '{0}'\n     {1}", filepath, e.what());
			return false;
		}
