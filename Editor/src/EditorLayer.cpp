...
namespace SLEngine {
    ...
    void EditorLayer::OnEvent(Event& e)
    {
		...
		dispatcher.Dispatch<MouseButtonPressedEvent>(SL_BIND_EVENT_FN(EditorLayer::OnMouseButtonPressed));
	}
	...
	{
		std::string filepath = FileDialogs::OpenFile("SLEngine Scene (*.scene)\0*.scene\0");
		if (!filepath.empty())
			OpenScene(filepath);
	}
	...
	void EditorLayer::OpenScene(const std::filesystem::path& path)
	{
		...
		{
			SL_WARN("Could not load {0} - not a scene file", path.filename().string());
			return;
		}
	...
	{
		std::string filepath = FileDialogs::SaveFile("SLEngine Scene (*.scene)\0*.scene\0");
		if (!filepath.empty())
		{
	...
	{
		SL_CORE_ASSERT(m_SceneState == SceneState::Play || m_SceneState == SceneState::Simulate);
