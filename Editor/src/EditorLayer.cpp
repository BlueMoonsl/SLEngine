...
namespace SLEngine {
	...
	{
		std::optional<std::string> filepath = FileDialogs::OpenFile("SLEngine Scene (*.scene)\0*.scene\0");
		if (filepath)
		{
			m_ActiveScene = CreateRef<Scene>();
			m_ActiveScene->OnViewportResize((uint32_t)m_ViewportSize.x, (uint32_t)m_ViewportSize.y);
	...
	{
		std::optional<std::string> filepath = FileDialogs::SaveFile("SLEngine Scene (*.scene)\0*.scene\0");
		if (filepath)
		{
			SceneSerializer serializer(m_ActiveScene);
